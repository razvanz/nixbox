...
assert_eq "empty ldLibraries" "$(jq_get "$json" '.nix.ldLibraries')" "[]"
...
assert_eq "user ldLibrary" "$(jq_get "$json" '.nix.ldLibraries[0]')" "libGL"
...
# ldLibraries: plugin contributes expat (user config only sets a plugin, no extras)
assert_eq "plugin ldLibrary" "$(jq_get "$json" '.nix.ldLibraries | index("expat") != null')" "true"
